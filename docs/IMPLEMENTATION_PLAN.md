...
1. **Finish the loop (pipeline)** — done: challenge-window scanning (`src/challenge/`) + the "already in registry" check now use a configurable Curate subgraph client (`src/utils/subgraph.ts`) and the scout-api; ERC-20/721/1155/EIP-1167 detection shipped; and `checkCompliance` now runs per-registry policy checks (ATR tags, token metadata/decimals/URLs, CDN domain + visual proof, ATQ repository). *(Set `SCOUT_SUBGRAPH_URL` to activate the subgraph path; the challenge module is now import-safe and unit-testable via the exported `checkCompliance`.)*
